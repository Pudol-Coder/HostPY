package com.github.promango.hostpy.managers;
...
public class ScriptManager {
    ...
    public ScriptManager(HostPY plugin) {
        this.plugin = plugin;
    ...
    public void startScript(String fileName, CommandSender sender) {
        ...
        if (runningProcesses.containsKey(key)) {
            sender.sendMessage("§c[HostPY] '" + fileName + "'은(는) 이미 실행 중입니다.");
            return;
        }

        File scriptFile = new File(scriptFolder, key);
        if (!scriptFile.exists()) {
            sender.sendMessage("§c[HostPY] scripts 폴더에서 '" + key + "' 파일을 찾을 수 없습니다.");
            return;
        }
        ...
        try {
            ...
            sender.sendMessage("§a[HostPY] '" + key + "' 실행을 시작했습니다.");

            process.onExit().thenRun(() -> {
                runningProcesses.remove(key);
                Bukkit.getScheduler().runTask(plugin, () ->
                        plugin.getLogger().info("[HostPY] '" + key + "' 프로세스가 종료되었습니다."));
            });
        } catch (IOException e) {
            sender.sendMessage("§c[HostPY] 실행 중 오류가 발생했습니다: " + e.getMessage());
    ...
    public void stopScript(String fileName, CommandSender sender) {
        ...
        if (process == null) {
            sender.sendMessage("§c[HostPY] '" + fileName + "'은(는) 실행 중이 아닙니다.");
            return;
        }

        process.destroy();
        runningProcesses.remove(key);
        sender.sendMessage("§a[HostPY] '" + key + "'을(를) 종료했습니다.");
    }

    public void listScripts(CommandSender sender) {
        if (runningProcesses.isEmpty()) {
            sender.sendMessage("§7[HostPY] 현재 실행 중인 스크립트가 없습니다.");
            return;
        }

        sender.sendMessage("§b[HostPY] 실행 중인 스크립트 (" + runningProcesses.size() + "개):");
    ...
    /** scripts 폴더에 있는 .py 파일 이름 목록 (확장자 제외, 탭완성용). */
    public java.util.List<String> getAvailableScriptNames() {
        File[] files = scriptFolder.listFiles((dir, name) -> name.endsWith(".py"));
    ...
    /** 현재 실행 중인 스크립트 이름 목록 (확장자 제외, 탭완성용). */
    public java.util.List<String> getRunningScriptNames() {
        return runningProcesses.keySet().stream()
                .map(name -> name.endsWith(".py") ? name.substring(0, name.length() - 3) : name)
                .toList();
    }
}
