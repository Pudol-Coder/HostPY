...
public class SignalManager {
    ...
    public SignalManager(HostPY plugin) {
        this.plugin = plugin;
    ...
    public void start() {
        ...
        watcherThread = new Thread(this::watchLoop, "HostPY-SignalWatcher");
    ...
    private void watchLoop() {
        try (WatchService watchService = FileSystems.getDefault().newWatchService()) {
            ...
            plugin.getLogger().warning("[HostPY] 신호 감시 중 오류: " + e.getMessage());
        }
    }
    ...
    private void handleIncomingSignal(File signalFile) {
        ...
        try {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                HostPYSignalEvent event = new HostPYSignalEvent(sourceScript, content);
                Bukkit.getPluginManager().callEvent(event);
            });
        } catch (IOException | InterruptedException e) {
            plugin.getLogger().warning("[HostPY] 신호 파일 처리 실패 (" + signalFile.getName() + "): " + e.getMessage());
        }
    }
    ...
    public void sendSignal(String content) {
        ...
        try {
            ...
            plugin.getLogger().warning("[HostPY] 신호 전송 실패: " + e.getMessage());
        }
    }
}
